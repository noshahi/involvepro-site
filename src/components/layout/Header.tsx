...
export function Header() {
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/" aria-label="Involvepro home" className="flex shrink-0 items-center gap-3">
          <Image
              ...
              {navLinks.map((item) => (
                <motion.div key={item.label} variants={{ hidden: { opacity: 0, x: -12 }, show: { opacity: 1, x: 0 } }}>
                  <Link
                    ...
                    className="block rounded-lg px-3 py-3 text-[15px] font-medium text-canvas-dark hover:bg-canvas-soft"
                  >
                    {item.label}
                  </Link>
                </motion.div>

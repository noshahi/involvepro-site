...
import { FileText, Layers, Clock3, CalendarCheck2, Users } from "lucide-react";
...
const floatingCards = [
  { icon: Layers, label: "Service Selector", className: "left-2 top-2 lg:left-[-20px] lg:top-8" },
  { icon: FileText, label: "Project Scope Card", className: "right-2 top-1/3 lg:right-[-26px]" },
  { icon: Clock3, label: "Timeline Card", className: "bottom-24 left-6 lg:bottom-28 lg:left-[-12px]" },
  { icon: CalendarCheck2, label: "Meeting Card", className: "bottom-2 right-8 lg:bottom-[-14px] lg:right-10" },
];

export function ContactHero() {
  return (
    <section aria-label="Contact" className="relative overflow-hidden bg-canvas-dark px-6 pb-20 pt-36 sm:pb-24 sm:pt-40">
      ...
      <div className="relative mx-auto grid max-w-[1280px] items-center gap-14 lg:grid-cols-[1.05fr_1fr] lg:gap-10">
        ...
        >
          <motion.div variants={{ hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0 } }}>
            <Pill tone="green">Contact</Pill>
          </motion.div>
          <motion.h1
            variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }}
            className="mt-6 max-w-[620px] font-sans text-[clamp(28px,4vw,46px)] font-bold leading-[1.1] tracking-[-1.5px] text-white"
          >
            Let&rsquo;s discuss what your website needs next.
          </motion.h1>
          <motion.p
            variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }}
            className="mt-6 max-w-[540px] text-[16px] leading-[1.65] text-white/60"
          >
            Tell us about your Shopify, WordPress, SEO, UI/UX, SaaS, full-stack, AI automation, or
            maintenance project. We will review the details and recommend the most practical next
            step.
          </motion.p>
          <motion.div
            variants={{ hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } }}
          ...
          >
            ...
            <MagneticButton href="#contact-form" variant="outline">
        ...
        >
          <BrowserFrame label="involvepro.com/contact">
